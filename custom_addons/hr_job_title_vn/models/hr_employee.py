...
# Higher index = higher role in the list (bottom = giám đốc).
...
class HrEmployee(models.Model):
    ...
    def _job_title_rank(self, job_title):
        """Return numeric rank for known titles; None skips hierarchy checks (unknown/custom titles)."""
    ...
    def _manager_rank_invalid_vs_employee(self):
        """True when manager's title rank is strictly below the employee's (when both are known)."""
        self.ensure_one()
    ...
    def _ensure_manager_not_below_subordinate(self):
        """Manager's job title must be on the same step or higher on the company scale than the employee."""
        self.ensure_one()
    ...
    def _onchange_warn_manager_job_title_order(self):
        """Small non-blocking notice while editing: manager must not have a lower-ranked title than the employee."""
        if not self._manager_rank_invalid_vs_employee():
            return
        labels = dict(JOB_TITLE_SELECTION)
    ...
    def _check_manager_job_title_hierarchy(self):
        for emp in self:
            ...
            # This person’s title changed: each direct report must still rank under their manager (this record).
